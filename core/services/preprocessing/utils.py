...
# TODO: проверяй amtr_(x, y, z) в сырых данных есть ли они, если нет 0,
def preprocess(df: pd.DataFrame, ANTI_BUG_TIME_SECONDS=10, PRE_PERIOD_TIME=3, PERIOD_2_MIN=30, VOLTAGE_LIMIT=.16,
               REFUELING_LIMIT=4000, is_debug=False) -> pd.DataFrame:
    if 'amtr_x' not in df:
        df['amtr_x'] = 0
    if 'amtr_y' not in df:
        df['amtr_y'] = 0
    ...
    pre_period_df['max_fuel'] = \
    pre_period_df.groupby([pd.Grouper(key='auto'), pd.Grouper(key='timestamp', freq=f'{PRE_PERIOD_TIME}min')])[
        'calc_sensors_fuel_level'].transform('max')

    pre_period_df = pre_period_df.groupby(
    ...
    pre_period_df['spent_fuel'].mask(
        pre_period_df['pos_s'].eq(0) & pre_period_df['spent_fuel'].gt(0.0) & pre_period_df['spent_fuel'].lt(
            REFUELING_LIMIT), np.nan, inplace=True)
...
def fuel_leak_calculate_standart(df_values: pd.DataFrame, norma_rasx_df: pd.DataFrame, LEAK_LIMIT = 12, SIGMA_LIMIT = 3.5, MULT_STD_MEAN_DIFF = 2.25, FUEL_JUMPS_AMOUNT = 30, LEAK_FACTOR = 0.05, SMALL_SPEED_FACTOR = 2.25, UNTARIFF_LEVEL = 400, is_save_bad_data = False):
    ...
    season_result['norma_rasx_per_travel'] = season_result['norma_rasx_per_travel'].mul(season_result['pos_s'].div(season_result['speed_etalon']))
    ...
    season_result['leak_factor'] = season_result['leak_factor'].mask(season_result['input'].eq(season_result['output']), season_result['max_fuel'].mul(LEAK_FACTOR))
    ...
    season_result['leak_std'].where(season_result['leak_std'].gt(season_result['leak_median'].mul(MULT_STD_MEAN_DIFF)), other=season_result['leak_std'], inplace=True)
    season_result['is_leak_2'] = season_result['leak_mean'].add(season_result['leak_std'].mul(SIGMA_LIMIT)).le(season_result['leak'])

...
def fuel_leak_calculate_standart(df_values: pd.DataFrame, norma_rasx_df: pd.DataFrame, LEAK_LIMIT = 20, SIGMA_LIMIT = 3, SPEED_ETALON = 60, FUEL_JIGGLE_FACTOR = 15, is_save_bad_data = False, is_filter = True, FILTER_PERC = .03):
    with warnings.catch_warnings():
        warnings.simplefilter(action='ignore')
        ...
        season_result['norma_rasx_per_travel'] = season_result['norma_rasx_per_travel'].mul(season_result['pos_s'].div(SPEED_ETALON).pow(2))
        ...
        season_result['is_leak'] = season_result['leak'].ge(LEAK_LIMIT) & season_result['leak'].lt(120)
        ...
        season_result['delta_sp_std'] = season_result['delta_sp_std'].div(3)
        ...
        season_result['is_leak'] = season_result['is_leak'] & season_result['pos_s'].lt(2) & season_result['is_tarrified']
        # новый фильтр

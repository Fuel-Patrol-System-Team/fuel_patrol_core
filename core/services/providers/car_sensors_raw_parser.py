...
class CarSensorsRawParser:
    ...
    def _process_fuel_mode(self, messages: List[Dict[str, Any]], sensors_mapping: Dict[str, str]) -> List[
        Dict[str, Any]]:
        """Обработка сообщений для режима топлива"""
        result = []
        ...
        for message in messages:
            try:
                ...
                fuel_level = self._get_nested_value(message, fuel_path)
                speed = self._get_nested_value(message, speed_path, 0.0)
                rpm = self._get_nested_value(message, rpm_path, 0.0)
                ...
                result.append({
                    "timestamp": timestamp.isoformat(),
                    "calc_sensors_fuel_level": float(fuel_level),
                    "pos_s": float(speed),
                    "rpm": float(rpm) if rpm is not None else 0.0
                })
                ...
                logger.debug(f"Ошибка обработки сообщения для режима топлива: {e}")
                continue

        return result

    def _process_motohours_mode(self, messages: List[Dict[str, Any]], sensors_mapping: Dict[str, str]) -> List[
        Dict[str, Any]]:
        """Обработка сообщений для режима моточасов"""
        result = []

        motohours_path = sensors_mapping.get("motohours", "")
        speed_path = sensors_mapping.get("speed", "speed")
        rpm_path = sensors_mapping.get("rpm", "")
        ign_path = sensors_mapping.get("ign", "")
        ...
        for message in messages:
            try:
                ...
                motohours = self._get_nested_value(message, motohours_path)
                speed = self._get_nested_value(message, speed_path, 0.0)
                rpm = self._get_nested_value(message, rpm_path, 0.0)
                ...
                if motohours is None:
                    continue

                result.append({
                    "timestamp": timestamp.isoformat(),
                    "motohours": float(motohours),
                    "pos_s": float(speed),
                    "rpm": float(rpm) if rpm is not None else 0.0,
                    "ign": ign
                })
                ...
                logger.debug(f"Ошибка обработки сообщения для режима моточасов: {e}")
                continue

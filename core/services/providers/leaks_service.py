...
class LeaksService(BaseLeaksCalculator):
    ...
    def compute_leaks(
        ...
        logger.info(f"📊 Входные данные: auto_df={len(auto_df)} машин, data_df={len(data_df)} записей, "
        ...
        for auto, data in data_df.partition_by("auto", as_dict=True).items():
            ...
            try:
                ...
                primary_info_record = primary_df.filter(pl.col("auto") == auto_id).to_dicts()
                norma_info_record = norma_df.filter(pl.col("sl_avto") == auto_id).to_dicts()

                logger.debug(f"Машина {auto_id}: найдено auto_info={len(auto_info_record)}, "
                ...
                logger.debug(f"Машина {auto_id}: auto_info keys={list(auto_info.keys())}")
                logger.debug(f"Машина {auto_id}: primary_info keys={list(primary_info.keys())}")
                logger.debug(f"Машина {auto_id}: norma_info keys={list(norma_info.keys())}")
                ...
                logger.error(f"💥 КРИТИЧЕСКАЯ ОШИБКА обработки машины {auto_id}: {e}", exc_info=True)
                skipped_cars += 1
                continue

        logger.info(f"📈 ИТОГИ ОБРАБОТКИ: обработано {processed_cars} машин, пропущено {skipped_cars}")
        ...
        if total_result is not None:
            ...
            logger.info(f"✅ Объединение завершено, финальный результат: {len(total_result)} записей")
        else:
    ...
    def _merge_with_additional_data(
        ...
        result_df = result_df.join(norma_df, left_on="auto", right_on="sl_avto", how="left")
        ...
        logger.debug(f"✅ Объединение завершено: {initial_count} -> {len(result_df)} записей")
        return result_df
    ...
    def _preprocess_basic(
        ...
        df = df.with_columns(
            pl.col("calc_sensors_fuel_level").diff().over("auto", col_dtime_2hour).fill_null(0).alias(
                "spent_fuel_clean")
        )
        ...
        initial_count = len(df)
        df = df.filter(pl.col("spent_fuel_clean").gt(-1))
        filtered_count = len(df)
        logger.debug(f"Фильтрация spent_fuel_clean: {initial_count} -> {filtered_count} записей")
        ...
        df = df.with_columns([
            ...
            (pl.col("calc_sensors_fuel_level") * pl.lit(cars['output']) / pl.lit(cars['input'])).alias(
        ...
        initial_count = len(df)
        df = df.filter(
            (pl.col("voltage_max") - pl.col("calc_sensors_voltage")) / pl.col("voltage_max") < VOLTAGE_LIMIT,
        )
        filtered_count = len(df)
        logger.debug(f"Фильтрация по напряжению: {initial_count} -> {filtered_count} записей")
        ...
        df = df.with_columns([
            ...
            pl.when((~pl.lit(primary['is_special_car']) & (pl.col("pos_s") == 0)) | (
        ...
        df = df.with_columns([
            ...
            pl.when(pl.col("spent_fuel").abs() > primary['max_fuel'] * FUEL_JUMP_BARRIER_PERC)
            .then(1)
            .otherwise(0)
            .cast(pl.Int16)
            .alias("jumps"),
        ...
        initial_count = len(df)
        df = (
            df.group_by_dynamic(index_column="timestamp", every=f"{ANTI_BUG_TIME_SECONDS}s", group_by="auto").agg([
        ...
        logger.debug(f"Группировка по {ANTI_BUG_TIME_SECONDS}с: {initial_count} -> {grouped_count} записей")
        ...
        df = df.with_columns(
            pl.when(
                ...
                |
                (pl.lit(primary['is_special_car']) & pl.lit(primary['ign_working']) & (pl.col("ign") == 0) & (
                        pl.col("pos_s") == 0)
                 & (pl.col("spent_fuel") > 0)
                 & (pl.col("spent_fuel") < REFUELING_LIMIT))
            )
        ...
        df = df.with_columns(
            pl.col("spent_fuel").rolling_mean_by('timestamp', window_size='30m', closed="both").alias(
                "spent_fuel_rolling")
        )
    ...
    def _preprocess(
        ...
        if initial_df is None:
            ...
            anti_bug = self._preprocess_basic(
                df, cars, norms, VOLTAGE_LIMIT, FUEL_JUMP_BARRIER_PERC,
                ANTI_BUG_TIME_SECONDS, REFUELING_LIMIT, PRE_PERIOD_TIME, DTIME_LIMIT
            )
        else:
        ...
        anti_bug = (
            anti_bug.group_by_dynamic(index_column="timestamp", every=f"{PRE_PERIOD_TIME}m", group_by="auto")
            .agg([
        ...
        logger.debug(f"Группировка {PRE_PERIOD_TIME}м: {initial_count} -> {grouped_count} записей")

        anti_bug = anti_bug.with_columns([
            pl.col("f1").diff().over(["auto"]).fill_null(0).cast(pl.Float32).alias("spent_fuel_standing"),
        ])
        logger.debug("Рассчитан spent_fuel_standing")

        anti_bug = anti_bug.with_columns((pl.col("pos_s") * (pl.col("dtime") / 3600)).alias("travel"))
        logger.debug("Рассчитано пройденное расстояние")

        logger.debug(f"Финальная группировка по {PERIOD_2_MIN}-минутным интервалам")
        initial_count = len(anti_bug)
        result = (
            anti_bug.group_by_dynamic(index_column="timestamp", every=f"{PERIOD_2_MIN}m", group_by="auto")
            .agg([
        ...
        logger.debug(f"Финальная группировка {PERIOD_2_MIN}м: {initial_count} -> {final_count} записей")
        ...
        logger.info(f"✅ Основная предобработка завершена: результат={len(result)}, промежуточные={len(anti_bug)}")
    ...
    def _fuel_leak_calculate(
        ...
        logger.info("🔍 Начало расчета утечек")
        logger.debug(f"Входные данные: {len(df_values)} записей, параметры: LEAK_LIMIT={LEAK_LIMIT}, "
        ...
        df_values = df_values.with_columns([
            (pl.when(pl.col("count") <= 5).then(pl.lit(1 << 0)).otherwise(0) |
             pl.when(pl.col("jumps") > FUEL_JUMPS_AMOUNT).then(pl.lit(1 << 1)).otherwise(0) |
             pl.when(pl.col("spent_fuel") > UNREASONABLE_FUEL_LEVEL).then(pl.lit(1 << 2)).otherwise(0)
             ).alias("bad_flags")
        ])
        logger.debug("Рассчитаны bad_flags")

        df_values = df_values.with_columns([
            pl.when(pl.col("spent_fuel") >= 0).then(0).otherwise(pl.col("spent_fuel")).alias("spent_fuel")
        ])
        df_values = df_values.with_columns(pl.col("spent_fuel").abs().alias("spent_fuel"))
        logger.debug("Обработан spent_fuel")

        df_values = df_values.with_columns([(pl.col("bad_flags") > 0).alias("is_bad_data")])
        bad_count = df_values.filter(pl.col("is_bad_data")).height
        logger.debug(f"Найдено плохих данных: {bad_count} записей")

        df_values = df_values.with_columns([(pl.col("fd") / pl.col("count")).alias("decrease_ratio")])
        ...
        if is_filter_bad_data:
            ...
            logger.debug(f"Фильтрация плохих данных: {initial_count} -> {filtered_count} записей")
        ...
        df_values = df_values.with_columns([
            (pl.col("norma_rasx") * (pl.col("pos_s") / norma['speed_etalon'])).pow(2)
        ...
        df_values = df_values.with_columns([
            (pl.col("spent_fuel") - pl.col("norma_rasx_per_travel")).clip(lower_bound=0).alias("leak")
        ])
        ...
        logger.info(f"✅ Расчет утечек завершен: всего утечек {leak_count} из {len(df_values)} записей")

...
class GlonassGeneralProvider:
    ...
    def parse_raw_data(self, mode: str, return_df=False, car : Car | None = None, start_date: datetime | None = None, end_date: datetime | None = None) -> tuple[ bool, pl.DataFrame | List[Dict[str, Any]] | None]:
        ...
        if mode == "mileage":
            result = self._process_general(car_to_use,all_messages, sensors_mapping, [GP.timestamp, GP.speed, GP.mileage, GP.satellites, GP.rpm, GP.ignition, GP.msg_number],[GLOBAL_GLONASS_ACTIONS.get(GL_ACTION_KEYS.auto_column)],  return_df=return_df)
        elif mode == "fuel":

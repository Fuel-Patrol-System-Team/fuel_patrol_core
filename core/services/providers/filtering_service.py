...
class FilteringService(BaseFilteringService):
    ...
    def apply_filters(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        filtered_df, _ = self.filtering_ptime(
            filtered_df,
        )
    ...
    def pick_boundary_spent_fuel(self, result_df: pl.DataFrame, HARD_LOSS_IN_BOUNDARY=15):
    ...
    def filtering_count(
        self, result_df: pl.DataFrame, COUNT_VALUE: int = 20
    ) -> Tuple[pl.DataFrame, pl.DataFrame]:

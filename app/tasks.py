...
from core.services.csv_parsing.utils import (
    PARSE_NORMS_OUTPUT_COLUMNS,
    parse_cars,
    parse_merged_util,
    parse_norms,
)
...
from core.services.preprocessing.utils import (
    fuel_leak_calculate_standart,
    preprocess,
    merge,
)
...
from core.services.providers.glonass.glonassoft_mileage_provider import (
    GlonassSoftMileageProvider,
)
...
@shared_task(bind=True, soft_time_limit=300, priority=5, rate_limit="1/s")
    ...
    def _fetch():
        ...
        try:
            ...
            provider = provider_factory(
                report_query_id, report_query.provider_id.metadata
            )
            if not provider:
            ...
            vehicles_data = provider.get_vehicles(
                name=None, start_date=start_date, end_date=end_date
            )
            ...
            logger.info(
                f"Задача fetch_data_from_provider завершена для {report_query_id}."
            )
            ...
            if report_query:
                ...
                report_query.save()
                try:
                    report_details = ReportQueryDetails.objects.get(
                        report_query_id=report_query_id
                    )
                    import traceback
    ...
    def _calculate():
        ...
        try:
            ...
            media = Media.objects.filter(
                report_query_id=report_query, type="raw"
            ).first()
            if not media or not media.file:
                logger.error(
                    f"Файл Media не найден для report_query_id={report_query_id}"
                )
                report_query.status = "error"
                report_query.save()
                return
            ...
            tariffied_cars = Car.objects.filter(is_tarrified=True).values(
                "id", "input", "output"
            )
            tariffied_df = pd.DataFrame(list(tariffied_cars)).rename(
                columns={"id": "guid"}
            )
            ...
            if tariffied_df.empty:
                logger.warning(
                    f"Нет тарированных машин для report_query_id={report_query_id}"
                )
                report_query.status = "completed"
                report_query.save()
                return
            ...
            for _, row in norms.iterrows():
                try:
                    ...
                    logger.error(
                        f"Ошибка сохранения норм для car_id={row['sl_avto']}: {e}"
                    )
                    continue
...
@shared_task(bind=True, soft_time_limit=1800, priority=2, rate_limit="2/m")
def process_raw_data_task(self, report_query_id: str):
    def _process():
        ...
        try:
            ...
            is_csv_provider = any(
                ext in provider.name.lower() for ext in excel_extensions
            )
            is_glonasssoft_provider = provider.name.lower() == "glonasssoft"

            logger.info(
                f"Провайдер: {provider.name}, csv: {is_csv_provider}, glonasssoft: {is_glonasssoft_provider}"
            )

            if is_csv_provider or is_glonasssoft_provider:
                if (
                    is_csv_provider
                    and not ReportQuery.objects.filter(
                        provider_id__org_id=organization, status="completed"
                    ).exists()
                ):
                    logger.error(
                        f"Данные автомобилей для {organization.name} не завершены"
                    )
                    report_query.status = "error"
                    report_query.save()
                    send_telegram_message(
                ...
                raw_media = Media.objects.filter(
                    report_query_id=report_query, type="raw"
                ).first()
                if not raw_media:
                ...
                if file_size > 10:
                    logger.warning(
                        f"Файл слишком большой ({file_size:.2f} ГБ). Возможны проблемы с памятью."
                    )
                ...
                if is_glonasssoft_provider:
                    cars = Car.objects.filter(
                        id__in=provider.cars.values_list("id", flat=True)
                    ).select_related()
                else:
                ...
                logger.info(
                    f"Car data: {car_data.shape}, тип: {type(car_data)}, колонки: {car_data.columns.tolist()}"
                )
                logger.info(f"Уникальные id: {car_data['id'].unique()[:5]}")

                if is_glonasssoft_provider:
                    consumptions = CarConsumption.objects.filter(
                        car_id__in=cars.values_list("id", flat=True)
                ...
                norma_df["car_id__id"] = (
                    norma_df["car_id__id"].astype(str).str.strip().str.lower()
                )
                norma_df = norma_df.rename(
                    columns={
                        "car_id__id": "sl_avto",
                ...
                try:
                    with open_func(raw_media.file.path, "rt", encoding="utf-8") as f:
                        ...
                        for chunk_idx, chunk_df in enumerate(raw_df):
                            ...
                            chunk_df["auto"] = (
                                chunk_df["auto"].astype(str).str.strip().str.lower()
                            )
                            logger.info(
                                f"Уникальные auto: {chunk_df['auto'].unique()[:5]}"
                            )

                            common_autos = set(chunk_df["auto"]).intersection(
                                set(car_data["id"])
                            )
                            logger.info(f"Общие auto с car_data: {len(common_autos)}")
                            if not common_autos:
                                logger.warning(
                                    f"Нет общих auto в чанке {chunk_idx + 1}"
                                )
                ...
                logger.info(
                    f"Тестовый вызов провайдера {provider.name} для {report_query_id}"
                )
        except SoftTimeLimitExceeded as e:
            logger.error(
                f"Превышено время в process_raw_data_task для {report_query_id}: {e}"
            )
            if report_query:
                report_query.status = "error"
                report_query.save()
...
@shared_task(bind=True, soft_time_limit=1000, priority=3)
def process_chunk(
    self,
    chunk_pickle: bytes,
    car_data_pickle: bytes,
    norma_data_pickle: bytes,
    report_query_id: str,
    org_id: str,
):
    def _process():
        try:
            ...
            logger.debug(
                f"Тип chunk_pickle: {type(chunk_pickle)}, размер: {len(chunk_pickle)} байт"
            )
            logger.debug(
                f"Тип car_data_pickle: {type(car_data_pickle)}, размер: {len(car_data_pickle)} байт"
            )
            logger.debug(
                f"Тип norma_data_pickle: {type(norma_data_pickle)}, размер: {len(norma_data_pickle)} байт"
            )
            ...
            logger.debug(
                f"Тип chunk_df после десериализации: {type(chunk_df)}, размер: {chunk_df.shape}"
            )
            logger.debug(
                f"Тип car_data_df после десериализации: {type(car_data_df)}, размер: {car_data_df.shape}"
            )
            logger.debug(
            ...
            if not isinstance(chunk_df, pd.DataFrame):
                logger.error(
                    f"Ожидался pandas.DataFrame для chunk_df, получен {type(chunk_df)}"
                )
                raise TypeError(
                    f"Ожидался pandas.DataFrame для chunk_df, получен {type(chunk_df)}"
                )
            if not isinstance(car_data_df, pd.DataFrame):
                logger.error(
                    f"Ожидался pandas.DataFrame для car_data_df, получен {type(car_data_df)}"
                )
                raise TypeError(
                    f"Ожидался pandas.DataFrame для car_data_df, получен {type(car_data_df)}"
                )
            if not isinstance(norma_data_df, pd.DataFrame):
                logger.error(
                    f"Ожидался pandas.DataFrame для norma_data_df, получен {type(norma_data_df)}"
                )
                raise TypeError(
                    f"Ожидался pandas.DataFrame для norma_data_df, получен {type(norma_data_df)}"
                )

            chunk_df["auto"] = chunk_df["auto"].apply(
                lambda x: str(x).replace("UUID:'", "").replace("'", "")
            )
            logger.info(
                f"Чанк: {chunk_df.shape}, тип: {type(chunk_df)}, колонки: {chunk_df.columns.tolist()}"
            )
            ...
            if not isinstance(preprocessed_df, pd.DataFrame):
                logger.error(
                    f"preprocess вернул {type(preprocessed_df)} вместо pandas.DataFrame"
                )
                raise TypeError(
                    f"preprocess вернул {type(preprocessed_df)} вместо pandas.DataFrame"
                )

            car_data_df["id"] = car_data_df["id"].apply(
                lambda x: str(x).replace("UUID:'", "").replace("'", "")
            )
            logger.info(
            ...
            logger.info(
                f"Объединено: {merged_df.shape}, тип: {type(merged_df)}, колонки: {merged_df.columns.tolist()}"
            )
            if not isinstance(merged_df, pd.DataFrame):
                logger.error(f"merge вернул {type(merged_df)} вместо pandas.DataFrame")
                raise TypeError(
                    f"merge вернул {type(merged_df)} вместо pandas.DataFrame"
                )
            if merged_df.empty:
                logger.warning(
                    f"merged_df пустой для {report_query_id}. Проверьте ключи id и auto."
                )
            ...
            result_df, bad_data_df = fuel_leak_calculate_standart(
                merged_df, norma_data_df, is_save_bad_data=is_save_bad_data
            )
            logger.info(
            ...
            if not isinstance(result_df, pd.DataFrame):
                logger.error(
                    f"fuel_leak_calculate_standart вернул {type(result_df)} вместо pandas.DataFrame"
                )
                raise TypeError(
                    f"fuel_leak_calculate_standart вернул {type(result_df)} вместо pandas.DataFrame"
                )
            leak_count = (
                result_df["is_leak"].sum() if "is_leak" in result_df.columns else 0
            )
            ...
            if is_save_bad_data and bad_data_df is not None and not bad_data_df.empty:
                logger.info(
                    f"Сохранение bad_data для {report_query_id}, строк: {len(bad_data_df)}"
                )
                cars = {
                    str(car.id).replace("UUID:'", "").replace("'", ""): car
                    for car in Car.objects.all().only("id")
                }
                with transaction.atomic():
                    for _, row in bad_data_df.iterrows():
                        try:
                            ...
                            if not car:
                                logger.warning(
                                    f"Автомобиль с id={car_id} не найден для bad_data"
                                )
                                continue
                            ...
                            logger.debug(
                                f"Сохранена запись CarBadData для car_id={car_id}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Ошибка сохранения CarBadData для car_id={car_id}: {e}"
                            )
                            continue
...
@shared_task(bind=True, soft_time_limit=300, priority=5)
def parse_cars_task(self, report_query_id):
    ...
    try:
        ...
        if is_csv_provider:
            auto_media = Media.objects.filter(
                report_query_id=report_query, type="auto"
            ).first()
            if not auto_media:
                logger.error(
            ...
            missing_columns = [
                col for col in required_columns if col not in parsed_cars.columns
            ]
            if missing_columns:
...
def parse_norms_task(self, report_query_id):
    ...
    try:
        ...
        if is_csv_provider:
            norm_media = Media.objects.filter(
                report_query_id=report_query, type="norm"
            ).first()
            if not norm_media:
            ...
            with transaction.atomic():
                for _, row in parsed_norms.iterrows():
                    try:
                        if not all(
                            pd.notna(row[col]) for col in PARSE_NORMS_OUTPUT_COLUMNS
                        ):
                            logger.warning(f"NaN в строке: {row}")
                            continue
                        ...
                        CarConsumption.objects.create(
                            car_id=car,
                            winter_volume=(
                                float(row["winter_norm"])
                                if pd.notna(row["winter_norm"])
                                else -1
                            ),
                            summer_volume=(
                                float(row["summer_norm"])
                                if pd.notna(row["summer_norm"])
                                else -1
                            ),
                            valid_period=(
                                pd.to_datetime(row["due"], errors="coerce").date()
                                if pd.notna(row["due"])
                                else None
                            ),
                        )
...
@shared_task(bind=True, soft_time_limit=1000, priority=4)
def save_leak_results(self, results, report_query_id):
    ...
    try:
        ...
        logger.info(
            f"Объединённый DataFrame: {result_df.shape}, колонки: {result_df.columns.tolist()}"
        )
        ...
        cars = {
            str(car.id).strip().lower(): car for car in Car.objects.all().only("id")
        }
        provider = report_query.provider_id
...
##TODO: Не используется
@shared_task(bind=True, soft_time_limit=1000, priority=0)
def check_and_process_raw_reports(self):
    ...
    try:
        ...
        for organization in organizations:
            report_query = (
                ReportQuery.objects.filter(
                    provider_id__org_id=organization, status="created"
                )
                .exclude(Q(status="completed") | Q(status="error"))
                .first()
            )
...
def parse_merged_data(self, report_query_id):
    ...
    try:
        ...
        logger.debug(
            f"Провайдер: {provider.id}, имя: {provider.name}, cars: {hasattr(provider, 'cars')}"
        )
                        ...
                        car, created = Car.objects.get_or_create(
                            ...
                            defaults={
                                ...
                                "description": (
                                    ...
                                    if pd.notna(obj.description)
                                    else ""
                                ),
                                "engine_type": (
                                    str(obj.engine_type).strip()
                                    if pd.notna(obj.engine_type)
                                    else 0.0
                                ),
                                "is_tarrified": True,
                            },
                ...
                for norm in norms:
                    try:
                        ...
                        CarConsumption.objects.create(
                            car_id=car,
                            winter_volume=(
                                float(norm.winter_norm)
                                if pd.notna(norm.winter_norm)
                                else -1
                            ),
                            summer_volume=(
                                float(norm.summer_norm)
                                if pd.notna(norm.summer_norm)
                                else -1
                            ),
                            valid_period=(
                                pd.to_datetime(norm.due, errors="coerce").date()
                                if pd.notna(norm.due)
                                else None
                            ),
                        )
                    except Car.DoesNotExist:
